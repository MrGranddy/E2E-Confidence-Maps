...
def visualize_predictions(images: torch.Tensor, conf_maps: torch.Tensor, preds: torch.Tensor, num_images_to_log: int = 4) -> List[torch.Tensor]:
    ...
    for i in range(num_images_to_log):
        ...
        img_tensor = torch.tensor(np.array(Image.open(buf)).transpose(2, 0, 1) / 255.0, dtype=torch.float32)
