...
def generate(cfg: DictConfig) -> None:
   ...
   # Create Experiment Directory
    ...
    model: pl.LightningModule = DirectPredictionModule.load_from_checkpoint(CHECKPOINT_PATH)
    ...
    patient_name = os.path.basename(os.path.dirname(IMAGE_PATH))  # Extract patient name from path
    slice_num = os.path.splitext(os.path.basename(IMAGE_PATH))[0]  # Extract slice number from path
...
def _plot_all_images(image_np: np.ndarray, pred_np: np.ndarray, conf_np: np.ndarray, diff_np: np.ndarray, save_path: str) -> None:
    """
